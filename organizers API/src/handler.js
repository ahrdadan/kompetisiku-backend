...
const createOrganizerHandler = async (request, h) => {
  ...
  await db.query('INSERT INTO organizers SET id =?, userId = (SELECT id FROM users WHERE id =?), username = (SELECT username FROM users WHERE id =?), organizerName =?, createdAt =?, updatedAt =?', [id, userId, userId, organizerName, createdAt, updatedAt])
  const isSuccess = await db.query('SELECT * FROM organizers WHERE id =?', [id])
...
const updateOrganizerByIdHandler = async (request, h) => {
  ...
  const { organizerName } = request.payload
  const { organizerId } = request.params
  const avail = await db.query('SELECT * FROM organizers WHERE id =?', [organizerId])
  if (avail.results.length > 0) {
    if (organizerName) {
      await db.query('UPDATE organizers SET organizerName =?, updatedAt =? WHERE id =?', [organizerName, updatedAt, organizerId])
      const { results } = await db.query('SELECT * FROM organizers WHERE id =?', [organizerId])
      const response = h.response({
        status: 'success',
        data: results[0]
...
const deleteOrganizerById = async (request, h) => {
  ...
  if (avail.results.length > 0) {
    ...
    const isSuccess = await db.query('SELECT * FROM organizers WHERE id =?', [organizerId])
